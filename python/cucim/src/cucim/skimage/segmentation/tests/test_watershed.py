...
class TestWatershed:
    ...
    def test_watershed_compact_basic(self):
        """Test basic compact watershed functionality."""
        # Create an image with two peaks
        image = cp.zeros((20, 20), dtype=cp.float32)
        image[5, 5] = 10
        image[14, 14] = 10
        image = ndi.gaussian_filter(image, sigma=2.0)
        ...
        labels = watershed(-image, markers, compactness=0.1)
    ...
    def test_watershed_compact_produces_rounder_regions(self):
        """Test that compact watershed produces more regular regions.

        With high compactness, regions should be more circular/compact.
        We test this indirectly by checking that the implementation runs
        ...
        # Create two markers
        markers = cp.zeros((50, 50), dtype=cp.int32)
        markers[10, 25] = 1  # top center
        markers[40, 25] = 2  # bottom center

        # Standard watershed (compactness=0)
        labels_standard = watershed(image, markers, compactness=0)

        # Compact watershed
        labels_compact = watershed(image, markers, compactness=1.0)

        # Both should produce valid segmentations
        assert cp.max(labels_standard) == 2
        assert cp.max(labels_compact) == 2

        # Both should have the same markers preserved
        assert labels_standard[10, 25] == 1
        assert labels_standard[40, 25] == 2
        assert labels_compact[10, 25] == 1
        assert labels_compact[40, 25] == 2

    def test_watershed_compact_with_mask(self):
        """Test compact watershed with a mask."""
        image = cp.ones((20, 20), dtype=cp.float32)
        ...
        labels = watershed(image, markers, mask=mask, compactness=0.5)

        # Check that only masked region is labeled
        assert labels[5, 10] == 1
        assert labels[14, 10] == 2
        assert labels[0, 0] == 0  # Outside mask

    def test_watershed_compact_connectivity_8(self):
        """Test compact watershed with 8-connectivity."""
        image = cp.ones((20, 20), dtype=cp.float32)
        ...
        labels = watershed(image, markers, connectivity=2, compactness=0.1)

        assert cp.max(labels) == 2
        assert labels[5, 5] == 1
        assert labels[14, 14] == 2

    def test_watershed_compact_values(self):
        """Test compact watershed with different compactness values."""
        image = cp.ones((30, 30), dtype=cp.float32)

        markers = cp.zeros((30, 30), dtype=cp.int32)
        markers[10, 15] = 1
        markers[20, 15] = 2

        # Test various compactness values
        for compactness in [0.001, 0.01, 0.1, 1.0, 10.0]:
            labels = watershed(image, markers, compactness=compactness)

            # Should produce valid segmentation
            assert cp.max(labels) == 2
            assert labels[10, 15] == 1
            assert labels[20, 15] == 2

    def test_watershed_compact_large_image(self):
        """Test compact watershed on a larger image for convergence."""
        ...
        labels = watershed(image, markers, compactness=0.5)
