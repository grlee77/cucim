...
def rescale(
    ...
    *,
    ...
    scale : {float, tuple of floats}
        Scale factors for spatial dimensions. Separate scale factors can be
        defined as (m, n[, ...]).
...
def _linear_polar_mapping(output_coords, k_angle, k_radius, center):
    ...
    -------
    coords : (M, 2) ndarray
        Array of `(col, row)` coordinates in the input image that correspond to
        the `output_coords` given as input.
    """
...
def _log_polar_mapping(output_coords, k_angle, k_radius, center):
    ...
    center : 2-tuple
        `(row, col)` coordinates that represent the center of the circle that
        bounds the area to be transformed in an input image.
...
def warp_polar(
    ...
    **kwargs,
    ...
    center : 2-tuple, optional
        `(row, col)` coordinates of the point in `image` that represents the
        center of the transformation (i.e., the origin in Cartesian space).
        Values can be of type `float`. If no value is given, the center is
        assumed to be the center point of `image`.
    radius : float, optional
