...
astro = color.rgb2gray(cp.array(data.astronaut()))

# -- filterbank1 on original image
patches1 = view_as_windows(astro, patch_shape)
patches1 = patches1.reshape(-1, patch_shape[0] * patch_shape[1])[::8]
# TODO: currently kmeans2 is being run on the CPU here
fb1, _ = kmeans2(cp.asnumpy(patches1), n_filters, minit='points')
...
fb2, _ = kmeans2(cp.asnumpy(patches2), n_filters, minit='points')
...
ax[0].imshow(cp.asnumpy(astro), cmap=plt.cm.gray)
ax[0].set_title("Image (original)")

ax[1].imshow(cp.asnumpy(fb1_montage), cmap=plt.cm.gray)
ax[1].set_title("K-means filterbank (codebook)\non original image")

ax[2].imshow(cp.asnumpy(astro_dog), cmap=plt.cm.gray)
ax[2].set_title("Image (LGN-like DoG)")

ax[3].imshow(cp.asnumpy(fb2_montage), cmap=plt.cm.gray)
