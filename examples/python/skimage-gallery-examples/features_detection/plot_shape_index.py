...
def create_test_image(
    ...
    """
    # generate using NumPy-based arrays for drawing disk, etc.
...
point_y, point_x = cp.where(cp.abs(s - target) < delta)
...
point_y_s, point_x_s = cp.where(cp.abs(s_smooth - target) < delta)
point_z_s = image[point_y_s, point_x_s]

# transfer to host for plotting
image = cp.asnumpy(image)
point_x, point_y, point_z = map(cp.asnumpy, (point_x, point_y, point_z))
point_x_s, point_y_s, point_z_s = map(cp.asnumpy, (point_x_s, point_y_s, point_z_s))
...
ax1.scatter(point_x, point_y, color='blue',
            **scatter_settings)
ax1.scatter(point_x_s, point_y_s, color='green',
            **scatter_settings)
...
ax3.imshow(cp.asnumpy(s), cmap=plt.cm.gray)
ax3.axis('off')
