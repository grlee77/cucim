...
from cucim.skimage.transform import rescale, resize, downscale_local_mean

rgb = cp.array(data.astronaut())
...
ax[1].imshow(cp.asnumpy(image_rescaled), cmap='gray')
ax[1].set_title("Rescaled image (aliasing)")

ax[2].imshow(cp.asnumpy(image_resized), cmap='gray')
ax[2].set_title("Resized image (no aliasing)")

ax[3].imshow(cp.asnumpy(image_downscaled), cmap='gray')
