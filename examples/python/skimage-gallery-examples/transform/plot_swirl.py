...
from cucim.skimage.transform import swirl


n_tile = 2
image = cp.tile(cp.array(data.checkerboard()), (n_tile, n_tile))
swirled = swirl(image, rotation=0, strength=10, radius=120*n_tile)

fig, (ax0, ax1) = plt.subplots(nrows=1, ncols=2, figsize=(8, 3),
                               sharex=True, sharey=True)

ax0.imshow(cp.asnumpy(image), cmap=plt.cm.gray)
ax0.axis('off')
ax1.imshow(cp.asnumpy(swirled), cmap=plt.cm.gray)
ax1.axis('off')
