...
from cucim.skimage.metrics import structural_similarity as ssim
...
noise = cp.ones_like(img) * 0.2 * (img.max() - img.min())
rng = cp.random.default_rng()
noise[rng.random(size=noise.shape) > 0.5] *= -1

img_noise = img + noise
img_const = img + cp.abs(noise)
...
ax[0].imshow(cp.asnumpy(img), cmap=plt.cm.gray, vmin=0, vmax=1)
ax[0].set_xlabel(f'MSE: {mse_none:.2f}, SSIM: {ssim_none:.2f}')
ax[0].set_title('Original image')

ax[1].imshow(cp.asnumpy(img_noise), cmap=plt.cm.gray, vmin=0, vmax=1)
ax[1].set_xlabel(f'MSE: {mse_noise:.2f}, SSIM: {ssim_noise:.2f}')
ax[1].set_title('Image with noise')

ax[2].imshow(cp.asnumpy(img_const), cmap=plt.cm.gray, vmin=0, vmax=1)
