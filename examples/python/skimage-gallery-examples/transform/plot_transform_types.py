...
img = img_as_float(cp.array(data.chelsea()))
tf_img = transform.warp(img, tform.inverse)
fig, ax = plt.subplots()
ax.imshow(cp.asnumpy(tf_img))
...
rotation = transform.EuclideanTransform(rotation=cp.pi/3)
shift = transform.EuclideanTransform(translation=-cp.array(img.shape[:2]) / 2)
# Compose transforms by multiplying their matrices
matrix = np.linalg.inv(shift.params) @ rotation.params @ shift.params
tform = transform.EuclideanTransform(matrix)
tf_img = transform.warp(img, tform.inverse)
fig, ax = plt.subplots()
_ = ax.imshow(cp.asnumpy(tf_img))
