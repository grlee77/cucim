...
ax[0].imshow(cp.asnumpy(text), cmap=plt.cm.gray)
ax[1].imshow(cp.asnumpy(rotated), cmap=plt.cm.gray)
ax[2].imshow(cp.asnumpy(back_rotated), cmap=plt.cm.gray)
...
src = cp.array([[0, 0], [0, 50], [300, 50], [300, 0]])
dst = cp.array([[155, 15], [65, 40], [260, 130], [360, 95]])
...
# transfer arrays to be plotted back to the host for Matplotlib
text = cp.asnumpy(text)
warped = cp.asnumpy(warped)
dst = cp.asnumpy(dst)
