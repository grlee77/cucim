...
from cucim.skimage.transform import warp_polar, rotate, rescale
from cucim.skimage.util import img_as_float

radius = 705
angle = 35
image = cp.array(data.retina())
...
ax[1].imshow(cp.asnumpy(rotated))
ax[2].set_title("Polar-Transformed Original")
ax[2].imshow(cp.asnumpy(image_polar))
ax[3].set_title("Polar-Transformed Rotated")
ax[3].imshow(cp.asnumpy(rotated_polar))
plt.show()
...
klog = radius / cp.log(radius)
shift_scale = 1 / (cp.exp(shiftc / klog))
...
from cucim.skimage.filters import window, difference_of_gaussians
...
image = rgb2gray(cp.array(data.retina()))
...
ax[0].imshow(cp.asnumpy(image), cmap='gray')
ax[1].set_title("Modified Image")
ax[1].imshow(cp.asnumpy(rts_image), cmap='gray')
ax[2].set_title("Log-Polar-Transformed Original")
ax[2].imshow(cp.asnumpy(warped_image))
ax[3].set_title("Log-Polar-Transformed Modified")
ax[3].imshow(cp.asnumpy(warped_rts))
...
image_fs = cp.abs(fftshift(fft2(wimage)))
rts_fs = cp.abs(fftshift(fft2(rts_wimage)))
...
klog = shape[1] / cp.log(radius)
shift_scale = cp.exp(shiftc / klog)

fig, axes = plt.subplots(2, 2, figsize=(8, 8))
ax = axes.ravel()
ax[0].set_title("Original Image FFT\n(magnitude; zoomed)")
center = [s // 2 for s in shape]
ax[0].imshow(cp.asnumpy(image_fs[center[0] - radius:center[0] + radius,
                                 center[1] - radius:center[1] + radius]),
             cmap='magma')
ax[1].set_title("Modified Image FFT\n(magnitude; zoomed)")
ax[1].imshow(cp.asnumpy(rts_fs[center[0] - radius:center[0] + radius,
                               center[1] - radius:center[1] + radius]),
             cmap='magma')
ax[2].set_title("Log-Polar-Transformed\nOriginal FFT")
ax[2].imshow(cp.asnumpy(warped_image_fs), cmap='magma')
ax[3].set_title("Log-Polar-Transformed\nModified FFT")
ax[3].imshow(cp.asnumpy(warped_rts_fs), cmap='magma')
