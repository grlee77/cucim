...
from cucim.skimage.util import view_as_blocks


# get astronaut from skimage.data in grayscale
l = color.rgb2gray(cp.array(data.astronaut()))
...
mean_view = cp.mean(flatten_view, axis=2)
max_view = cp.max(flatten_view, axis=2)
median_view = cp.median(flatten_view, axis=2)
...
ax[0].imshow(cp.asnumpy(l_resized), extent=(-0.5, 128.5, 128.5, -0.5),
             cmap=cm.Greys_r)

ax[1].set_title("Block view with\n local mean pooling")
ax[1].imshow(cp.asnumpy(mean_view), cmap=cm.Greys_r)

ax[2].set_title("Block view with\n local max pooling")
ax[2].imshow(cp.asnumpy(max_view), cmap=cm.Greys_r)

ax[3].set_title("Block view with\n local median pooling")
ax[3].imshow(cp.asnumpy(median_view), cmap=cm.Greys_r)
