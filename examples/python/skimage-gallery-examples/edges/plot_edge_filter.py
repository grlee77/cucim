...
axes[0].imshow(cp.asnumpy(edge_roberts), cmap=plt.cm.gray)
axes[0].set_title('Roberts Edge Detection')

axes[1].imshow(cp.asnumpy(edge_sobel), cmap=plt.cm.gray)
...
x, y = cp.ogrid[:100, :100]

# Creating a rotation-invariant image with different spatial frequencies.
image_rot = cp.exp(1j * cp.hypot(x, y) ** 1.3 / 20.).real

edge_sobel = filters.sobel(image_rot)
edge_scharr = filters.scharr(image_rot)
edge_prewitt = filters.prewitt(image_rot)

diff_scharr_prewitt = compare_images(cp.asnumpy(edge_scharr), cp.asnumpy(edge_prewitt))
diff_scharr_sobel = compare_images(cp.asnumpy(edge_scharr), cp.asnumpy(edge_sobel))
...
axes[0].imshow(cp.asnumpy(image_rot), cmap=plt.cm.gray)
axes[0].set_title('Original image')

axes[1].imshow(cp.asnumpy(edge_scharr), cmap=plt.cm.gray)
axes[1].set_title('Scharr Edge Detection')

axes[2].imshow(diff_scharr_prewitt,
               cmap=plt.cm.gray, vmax=max_diff)
...
x, y = cp.mgrid[-10:10:255j, -10:10:255j]
image_rotinv = cp.sin(x ** 2 + y ** 2)

image_x = 2 * x * cp.cos(x ** 2 + y ** 2)
image_y = 2 * y * cp.cos(x ** 2 + y ** 2)

def angle(dx, dy):
    """Calculate the angles between horizontal and vertical operators."""
    return cp.mod(cp.arctan2(dy, dx), cp.pi)
...
def diff_angle(angle_1, angle_2):
    ...
    return cp.minimum(cp.pi - cp.abs(angle_1 - angle_2),
                      cp.abs(angle_1 - angle_2))
...
axes[0].imshow(cp.asnumpy(image_rotinv), cmap=plt.cm.gray)
axes[0].set_title('Original image')

axes[1].imshow(cp.asnumpy(true_angle), cmap=plt.cm.hsv)
axes[1].set_title('Analytical gradient angle')

axes[2].imshow(cp.asnumpy(diff_sobel), cmap=plt.cm.inferno, vmin=0, vmax=0.02)
axes[2].set_title('Sobel error')

axes[3].imshow(cp.asnumpy(diff_prewitt), cmap=plt.cm.inferno,
               vmin=0, vmax=0.02)
axes[3].set_title('Prewitt error')

axes[4].imshow(cp.asnumpy(diff_scharr), cmap=plt.cm.inferno, vmin=0, vmax=0.02)
axes[4].set_title('Scharr error')

color_ax = axes[5].imshow(cp.asnumpy(diff_farid), cmap=plt.cm.inferno,
                          vmin=0, vmax=0.02)
