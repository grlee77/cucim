...
from skimage import data as data_cpu

from cucim.skimage import data
from cucim.skimage.morphology import medial_axis, thin  # , skeletonize
from cucim.skimage.util import invert

# Invert the horse image
image = invert(cp.array(data_cpu.horse()))
...
ax[0].imshow(cp.asnumpy(blobs), cmap=plt.cm.gray)
ax[0].set_title('original')
ax[0].axis('off')

ax[1].imshow(cp.asnumpy(dist_on_skel), cmap='magma')
ax[1].contour(cp.asnumpy(blobs), [0.5], colors='w')
ax[1].set_title('medial_axis')
ax[1].axis('off')

# ax[2].imshow(skeleton, cmap=plt.cm.gray)
ax[2].set_title('skeletonize (TODO)')
ax[2].axis('off')

# ax[3].imshow(skeleton_lee, cmap=plt.cm.gray)
ax[3].set_title("skeletonize (Lee 94) (TODO)")
ax[3].axis('off')
...
# ax[1].imshow(skeleton, cmap=plt.cm.gray)
ax[1].set_title('skeleton (TODO)')
ax[1].axis('off')

ax[2].imshow(cp.asnumpy(thinned), cmap=plt.cm.gray)
ax[2].set_title('thinned')
ax[2].axis('off')

ax[3].imshow(cp.asnumpy(thinned_partial), cmap=plt.cm.gray)
