...
from cucim.skimage.segmentation import (morphological_chan_vese,
...
ax[0].set_axis_off()
ax[0].contour(cp.asnumpy(ls), [0.5], colors='r')
ax[0].set_title("Morphological ACWE segmentation", fontsize=12)

ax[1].imshow(cp.asnumpy(ls), cmap="gray")
ax[1].set_axis_off()
contour = ax[1].contour(cp.asnumpy(evolution[2]), [0.5], colors='g')
contour.collections[0].set_label("Iteration 2")
contour = ax[1].contour(cp.asnumpy(evolution[7]), [0.5], colors='y')
contour.collections[0].set_label("Iteration 7")
contour = ax[1].contour(cp.asnumpy(evolution[-1]), [0.5], colors='r')
...
# Morphological GAC
image = img_as_float(cp.array(data.coins()))
gimage = inverse_gaussian_gradient(image)

# Initial level set
init_ls = cp.zeros(image.shape, dtype=np.int8)
...
ax[2].imshow(cp.asnumpy(image), cmap="gray")
ax[2].set_axis_off()
ax[2].contour(cp.asnumpy(ls), [0.5], colors='r')
ax[2].set_title("Morphological GAC segmentation", fontsize=12)

ax[3].imshow(cp.asnumpy(ls), cmap="gray")
ax[3].set_axis_off()
contour = ax[3].contour(cp.asnumpy(evolution[0]), [0.5], colors='g')
contour.collections[0].set_label("Iteration 0")
contour = ax[3].contour(cp.asnumpy(evolution[100]), [0.5], colors='y')
contour.collections[0].set_label("Iteration 100")
contour = ax[3].contour(cp.asnumpy(evolution[-1]), [0.5], colors='r')
