...
# copy all tables to the host for use in pandas
tables = [{k: cp.asnumpy(v) for k, v in table.items()} for table in tables]
...
ax1.imshow(cp.asnumpy(images[0]), cmap='gray_r')
ax1.set_axis_off()
ax1.set_title(f'fraction {fractions[0]}')
# Show image with highest volume fraction
ax2.imshow(cp.asnumpy(images[-1]), cmap='gray_r')
ax2.set_axis_off()
