...
data = cucim.skimage.img_as_float(binary_blobs(length=128, seed=1))
sigma = 0.35
data += rng.standard_normal(data.shape) * sigma
data = rescale_intensity(data, in_range=(-sigma, 1 + sigma),
                         out_range=(-1, 1))

# The range of the binary image spans over (-1, 1).
# We choose the hottest and the coldest pixels as markers.
markers = cp.zeros(data.shape, dtype=cp.uint)
markers[data < -0.95] = 1
markers[data > 0.95] = 2
...
ax1.imshow(cp.asnumpy(data), cmap='gray')
ax1.axis('off')
ax1.set_title('Noisy data')
ax2.imshow(cp.asnumpy(markers), cmap='magma')
ax2.axis('off')
ax2.set_title('Markers')
ax3.imshow(cp.asnumpy(labels), cmap='gray')
ax3.axis('off')
