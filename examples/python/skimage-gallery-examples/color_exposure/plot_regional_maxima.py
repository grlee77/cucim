...
ax0.imshow(cp.asnumpy(image), cmap='gray')
ax0.set_title('original image')
ax0.axis('off')

ax1.imshow(cp.asnumpy(dilated),
           vmin=float(image.min()), vmax=float(image.max()), cmap='gray')
ax1.set_title('dilated')
ax1.axis('off')

ax2.imshow(cp.asnumpy(image - dilated), cmap='gray')
...
ax0.plot(cp.asnumpy(mask[yslice]), '0.5', label='mask')
ax0.plot(cp.asnumpy(seed[yslice]), 'k', label='seed')
ax0.plot(cp.asnumpy(dilated[yslice]), 'r', label='dilated')
ax0.set_ylim(-0.2, 2)
ax0.set_title('image slice')
ax0.set_xticks([])
ax0.legend()

ax1.imshow(cp.asnumpy(dilated), vmin=image.min(), vmax=image.max(), cmap='gray')
ax1.axhline(yslice, color='r', alpha=0.4)
ax1.set_title('dilated')
ax1.axis('off')

ax2.imshow(cp.asnumpy(hdome), cmap='gray')
