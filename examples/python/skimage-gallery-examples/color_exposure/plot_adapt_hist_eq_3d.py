...
verts = np.array([[i, j, k] for i in [0, 1]
                  for j in [0, 1] for k in [0, 1]]).astype(cp.float32)
lines = [np.array([i, j]) for i in verts
         for j in verts if cp.allclose(cp.linalg.norm(i - j), 1)]
...
axs[3].scatter(
    ...
    s=5,
    c=scalars_to_rgba(sigmoid, cmap=cmap, vmin=0, vmax=1, alpha=1.)[:, :3],
)
