...
grayscale_image = img_as_float(cp.array(data.camera()[::2, ::2]))
image = color.gray2rgb(grayscale_image)

red_multiplier = cp.array([1, 0, 0])
yellow_multiplier = cp.array([1, 1, 0])

fig, (ax1, ax2) = plt.subplots(ncols=2, figsize=(8, 4),
                               sharex=True, sharey=True)
ax1.imshow(cp.asnumpy(red_multiplier * image))
ax2.imshow(cp.asnumpy(yellow_multiplier * image))
...
hue_gradient = cp.linspace(0, 1)
hsv = cp.ones(shape=(1, len(hue_gradient), 3), dtype=float)
hsv[:, :, 0] = hue_gradient
...
ax.imshow(cp.asnumpy(all_hues), extent=(0 - 0.5 / len(hue_gradient),
...
for ax, hue in zip(axes.flat, hue_rotations):
    ...
    ax.imshow(cp.asnumpy(tinted_image), vmin=0, vmax=1)
    ax.set_axis_off()
fig.tight_layout()
