...
ax_each.imshow(cp.asnumpy(rescale_intensity(1 - sobel_each(image))))
ax_each.set_xticks([]), ax_each.set_yticks([])
ax_each.set_title("Sobel filter computed\n on individual RGB channels")

# We use 1 - sobel_hsv(image) but this won't work if image is not normalized
ax_hsv.imshow(cp.asnumpy(rescale_intensity(1 - sobel_hsv(image))))
...
ax.imshow(
    cp.asnumpy(rescale_intensity(1 - sobel_gray(image))), cmap=plt.cm.gray
)
