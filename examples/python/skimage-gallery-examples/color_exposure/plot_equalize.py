...
def plot_img_and_hist(image, axes, bins=256):
    ...
    # Display image
    ax_img.imshow(cp.asnumpy(image), cmap=plt.cm.gray)
    ax_img.set_axis_off()

    # Display histogram
    ax_hist.hist(cp.asnumpy(image.ravel()), bins=bins, histtype='step',
                 color='black')
    ...
    ax_cdf.plot(cp.asnumpy(bins), cp.asnumpy(img_cdf), 'r')
    ax_cdf.set_yticks([])
...
img = cp.array(data.moon())

# Contrast stretching
p2, p98 = map(float, cp.percentile(img, (2, 98)))
