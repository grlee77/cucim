...
ax0.imshow(cp.asnumpy(rgb_img))
ax0.set_title("RGB image")
ax0.axis('off')
ax1.imshow(cp.asnumpy(hue_img), cmap='hsv')
ax1.set_title("Hue channel")
ax1.axis('off')
ax2.imshow(cp.asnumpy(value_img))
...
ax0.hist(cp.asnumpy(hue_img.ravel()), 512)
ax0.set_title("Histogram of the Hue channel with threshold")
ax0.axvline(x=hue_threshold, color='r', linestyle='dashed', linewidth=2)
ax0.set_xbound(0, 0.12)
ax1.imshow(cp.asnumpy(binary_img))
...
ax0.imshow(cp.asnumpy(binary_img))
