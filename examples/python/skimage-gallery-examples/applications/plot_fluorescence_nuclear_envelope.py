...
image_sequence = cp.array(protein_transport())
...
mask = cp.logical_and(dilate, ~erode)
...
ax[0, 0].imshow(cp.asnumpy(image_t_0_channel_0), cmap=plt.cm.gray)
ax[0, 0].set_title('a) Raw')

ax[0, 1].imshow(cp.asnumpy(smooth), cmap=plt.cm.gray)
ax[0, 1].set_title('b) Blur')

ax[0, 2].imshow(cp.asnumpy(thresh), cmap=plt.cm.gray)
ax[0, 2].set_title('c) Threshold')

ax[0, 3].imshow(cp.asnumpy(fill), cmap=plt.cm.gray)
ax[0, 3].set_title('c-1) Fill in')

ax[1, 0].imshow(cp.asnumpy(clear), cmap=plt.cm.gray)
ax[1, 0].set_title('c-2) Keep one nucleus')

ax[1, 1].imshow(cp.asnumpy(dilate), cmap=plt.cm.gray)
ax[1, 1].set_title('d) Dilate')

ax[1, 2].imshow(cp.asnumpy(erode), cmap=plt.cm.gray)
ax[1, 2].set_title('e) Erode')

ax[1, 3].imshow(cp.asnumpy(mask), cmap=plt.cm.gray)
...
selection = cp.where(mask, image_t_0_channel_1, 0)

fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(12, 6), sharey=True)

ax0.imshow(cp.asnumpy(image_t_0_channel_1))
ax0.set_title('Second channel (raw)')
ax0.set_axis_off()

ax1.imshow(cp.asnumpy(selection))
...
thresh_seq = cp.stack([smooth_seq[k, ...] > val
                       for k, val in enumerate(thresh_values)])
...
border_mask = cp.ones_like(fill_seq)
...
mask_sequence = cp.logical_and(dilate_seq, ~erode_seq)
...
label_dtype = cp.min_scalar_type(n_z)
mask_sequence = mask_sequence.astype(label_dtype)
labels = cp.arange(1, n_z + 1, dtype=label_dtype)
...
fluorescence_change = np.asarray([props['area'][i] * props['intensity_mean'][i]
                                  for i in range(n_z)])
