...
from cucim.skimage.filters import (threshold_mean, threshold_minimum,
                                   threshold_otsu, threshold_local,
                                   try_all_threshold)
# from cucim.skiage.filters import rank
from cucim.skimage.util import img_as_ubyte


img = cp.array(data.page())
...
ax[1].imshow(cp.asnumpy(binary), cmap=plt.cm.gray)
...
ax[0, 0].imshow(cp.asnumpy(image), cmap=plt.cm.gray)
ax[0, 0].set_title('Original')

ax[0, 1].hist(cp.asnumpy(image.ravel()), bins=256)
ax[0, 1].set_title('Histogram')

ax[1, 0].imshow(cp.asnumpy(binary_min), cmap=plt.cm.gray)
ax[1, 0].set_title('Thresholded (min)')

ax[1, 1].hist(cp.asnumpy(image.ravel()), bins=256)
ax[1, 1].axvline(cp.asnumpy(thresh_min), color='r')
...
ax[1].hist(cp.asnumpy(image.ravel()), bins=256)
ax[1].set_title('Histogram')
ax[1].axvline(cp.asnumpy(thresh), color='r')

ax[2].imshow(cp.asnumpy(binary), cmap=plt.cm.gray)
...
ax[0].imshow(cp.asnumpy(image))
ax[0].set_title('Original')

ax[1].imshow(cp.asnumpy(binary_global))
ax[1].set_title('Global thresholding')

ax[2].imshow(cp.asnumpy(binary_local))
...
img = img_as_ubyte(cp.array(data.page()))
...
fig.colorbar(ax[0].imshow(cp.asnumpy(img), cmap=plt.cm.gray),
             ax=ax[0], orientation='horizontal')
ax[0].set_title('Original')
ax[0].axis('off')

# fig.colorbar(ax[1].imshow(cp.asnumpy(local_otsu), cmap=plt.cm.gray),
#              ax=ax[1], orientation='horizontal')
ax[1].set_title('Local Otsu (radius=%d) (TODO)' % radius)
ax[1].axis('off')

# ax[2].imshow(img >= local_otsu, cmap=plt.cm.gray)
ax[2].set_title('Original >= Local Otsu (TODO)' % threshold_global_otsu)
ax[2].axis('off')

ax[3].imshow(cp.asnumpy(global_otsu), cmap=plt.cm.gray)
