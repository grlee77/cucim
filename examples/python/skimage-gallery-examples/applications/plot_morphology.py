...
from cucim.skimage.morphology import (erosion, dilation, opening, closing,
                                      white_tophat, black_tophat, disk)
from cucim.skimage.morphology import thin
# from cucim.skimage.morphology import skeletonize, convex_hull_image
from cucim.skimage.util import img_as_ubyte


orig_phantom = img_as_ubyte(cp.array(data.shepp_logan_phantom()))
fig, ax = plt.subplots()
ax.imshow(cp.asnumpy(orig_phantom), cmap=plt.cm.gray)
...
def plot_comparison(original, filtered, filter_name):
    ...
    fig, (ax1, ax2) = plt.subplots(ncols=2, figsize=(8, 4), sharex=True,
                                   sharey=True)
    ax1.imshow(cp.asnumpy(original), cmap=plt.cm.gray)
    ax1.set_title('original')
    ax1.axis('off')
    ax2.imshow(cp.asnumpy(filtered), cmap=plt.cm.gray)
...
# TODO: use skeletonize once implemented
if False:
    sk = skeletonize(horse == 0)
    plot_comparison(horse, sk, 'skeletonize')
else:
    sk = thin(horse == 0)
    plot_comparison(horse, sk, 'thinned')
...
# TODO: implement convex hull on the GPU
