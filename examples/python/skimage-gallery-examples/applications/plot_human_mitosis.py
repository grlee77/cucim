...
image_cpu = human_mitosis()
image = cp.array(image_cpu)
...
qcs = ax.contour(image_cpu, origin='image')
...
ax[0].imshow(image_cpu)
ax[0].set_title('Original')
ax[0].axis('off')
ax[1].imshow(cp.asnumpy(regions))
...
ax[0].set_title('Original')
ax[0].axis('off')
ax[2].imshow(cp.asnumpy(cells))
ax[2].set_title('All nuclei?')
ax[2].axis('off')
ax[1].imshow(cp.asnumpy(dividing))
...
# TODO: use filters.rank.mean once implemented
if False:
    smoother_dividing = filters.rank.mean(util.img_as_ubyte(dividing),
                                          morphology.disk(4))
else:
    footprint = morphology.disk(4).astype(np.float32)
    footprint /= footprint.size
    smoother_dividing = ndi.convolve(util.img_as_ubyte(dividing), footprint)

binary_smoother_dividing = smoother_dividing > 20

fig, ax = plt.subplots(figsize=(5, 5))
ax.imshow(cp.asnumpy(binary_smoother_dividing))
...
local_max_mask = cp.zeros(distance.shape, dtype=bool)
local_max_mask[tuple(local_max_coords.T)] = True
markers = measure.label(local_max_mask)

# TODO: implement watershed on the GPU
from skimage.segmentation import watershed as watershed_cpu
segmented_cells = cp.array(
    watershed_cpu(
        -cp.asnumpy(distance), cp.asnumpy(markers), mask=cp.asnumpy(cells)
    )
)

fig, ax = plt.subplots(ncols=2, figsize=(10, 5))
ax[0].imshow(cp.asnumpy(cells), cmap='gray')
ax[0].set_title('Overlapping nuclei')
ax[0].axis('off')
ax[1].imshow(cp.asnumpy(color.label2rgb(segmented_cells, bg_label=0)))
...
fig, ax = plt.subplots(figsize=(5, 5))
ax.imshow(cp.asnumpy(color_labels))
