...
from cucim.skimage.filters import difference_of_gaussians, window
from cupyx.scipy.fft import fftn, fftshift

image = cp.array(gravel())
wimage = image * window('hann', image.shape)  # window image to improve FFT
filtered_image = difference_of_gaussians(image, 1, 12)
filtered_wimage = filtered_image * window('hann', image.shape)
im_f_mag = fftshift(cp.abs(fftn(wimage)))
fim_f_mag = fftshift(cp.abs(fftn(filtered_wimage)))

fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(8, 8))
ax[0, 0].imshow(cp.asnumpy(image), cmap='gray')
ax[0, 0].set_title('Original Image')
ax[0, 1].imshow(cp.asnumpy(cp.log(im_f_mag)), cmap='magma')
ax[0, 1].set_title('Original FFT Magnitude (log)')
ax[1, 0].imshow(cp.asnumpy(filtered_image), cmap='gray')
ax[1, 0].set_title('Filtered Image')
ax[1, 1].imshow(cp.asnumpy(cp.log(fim_f_mag)), cmap='magma')
