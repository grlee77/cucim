...
image = img_as_float(rgb2gray(cp.array(astronaut())))

wimage = image * window('hann', image.shape)

image_f = cp.abs(fftshift(fft2(image)))
wimage_f = cp.abs(fftshift(fft2(wimage)))
...
ax[1].imshow(cp.asnumpy(wimage), cmap='gray')
ax[2].set_title("Original FFT (frequency)")
ax[2].imshow(cp.asnumpy(cp.log(image_f)), cmap='magma')
ax[3].set_title("Window + FFT (frequency)")
ax[3].imshow(cp.asnumpy(cp.log(wimage_f)), cmap='magma')
plt.show()
